...
import {
  ArrowLeft, ArrowRight, BarChart3, Bell, BookOpen, CalendarDays, CheckCircle2,
    ...
    <header className="sticky top-0 z-40 border-b border-slate-200/70 bg-white/85 backdrop-blur-xl dark:border-white/10 dark:bg-slate-950/80">
      <div className="mx-auto flex h-20 max-w-[1500px] items-center gap-4 px-5 lg:px-10">
        ...
        <div className="relative mx-auto w-full max-w-2xl">
          ...
          <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search Atlas..." aria-label="Search Atlas" className="w-full rounded-2xl border border-slate-200/80 bg-slate-50/80 py-3 pl-11 pr-20 text-sm outline-none transition focus:border-indigo-400 focus:bg-white focus:ring-4 focus:ring-indigo-100/70 dark:border-slate-700 dark:bg-slate-900 dark:focus:ring-indigo-950" />
        ...
        <button className="relative hidden h-11 w-11 shrink-0 place-items-center rounded-xl border border-slate-200 hover:bg-slate-50 sm:grid dark:border-slate-700 dark:hover:bg-slate-800" aria-label="Notifications"><Bell className="h-4.5 w-4.5" /><span className="absolute right-2.5 top-2.5 h-2 w-2 rounded-full border-2 border-white bg-rose-500 dark:border-slate-950" /></button>
...
function HomePage() {
  return <div className="relative mx-auto max-w-[1500px] px-5 py-8 lg:px-10 lg:py-12">
    ...
    <section className="relative overflow-hidden rounded-[2rem] bg-[#0b1020] px-7 py-12 text-white shadow-[0_30px_80px_-30px_rgba(49,46,129,.5)] sm:px-12 lg:px-16 lg:py-16"><div className="absolute inset-0 bg-[radial-gradient(circle_at_75%_15%,rgba(99,102,241,.35),transparent_35%),radial-gradient(circle_at_85%_90%,rgba(34,211,238,.15),transparent_35%)]" /><div className="absolute right-[8%] top-1/2 hidden h-64 w-64 -translate-y-1/2 rounded-full border border-white/10 lg:block"><div className="absolute inset-8 rounded-full border border-white/10" /><div className="absolute inset-16 rounded-full border border-indigo-400/30 bg-indigo-500/10 backdrop-blur" /><Sparkles className="absolute left-1/2 top-1/2 h-9 w-9 -translate-x-1/2 -translate-y-1/2 text-indigo-300" /><span className="absolute -left-10 top-10 rounded-xl border border-white/10 bg-white/10 px-4 py-2 text-[10px] font-bold uppercase tracking-widest backdrop-blur">Learn</span><span className="absolute -right-8 bottom-12 rounded-xl border border-white/10 bg-white/10 px-4 py-2 text-[10px] font-bold uppercase tracking-widest backdrop-blur">Grow</span></div><div className="relative max-w-3xl"><span className="mb-6 inline-flex items-center gap-2 rounded-full border border-indigo-300/20 bg-white/[.07] px-3 py-1.5 text-[11px] font-bold uppercase tracking-[.15em] text-indigo-200 backdrop-blur"><span className="h-1.5 w-1.5 rounded-full bg-cyan-400" /> Your command centre</span><h1 className="max-w-2xl text-4xl font-extrabold leading-[1.05] tracking-[-.045em] sm:text-6xl lg:text-7xl">Everything you need to move <span className="bg-gradient-to-r from-indigo-300 via-violet-300 to-cyan-300 bg-clip-text text-transparent">forward.</span></h1><p className="mt-6 max-w-xl text-base leading-7 text-slate-300 sm:text-lg">Company intelligence, practical training, approved marketing and essential resources—curated for exceptional agents.</p><div className="mt-8 flex flex-wrap gap-3"><a href="/training/on-going/" className="inline-flex items-center gap-2 rounded-xl bg-white px-5 py-3 text-sm font-bold text-slate-950 transition hover:-translate-y-0.5 hover:shadow-xl">Explore training <ArrowRight className="h-4 w-4" /></a><a href="/resources/general/" className="inline-flex items-center gap-2 rounded-xl border border-white/15 bg-white/[.06] px-5 py-3 text-sm font-bold text-white backdrop-blur transition hover:bg-white/10">Browse resources</a></div></div></section>
    <HomeDashboard />
    <div className="mb-6 mt-16 flex items-end justify-between"><div><p className="text-[11px] font-bold uppercase tracking-[.2em] text-indigo-600">Your workspace</p><h2 className="mt-1 text-3xl font-extrabold tracking-[-.03em]">Explore Atlas</h2></div><span className="hidden text-sm text-slate-500 sm:block">23 curated agent resources</span></div>
    <div className="relative grid gap-5 md:grid-cols-2 xl:grid-cols-3">{categories.map((category) => { const Icon = category.icon; const theme = categoryTheme[category.name]; return <section key={category.name} className="group relative overflow-hidden rounded-3xl border border-slate-200/80 bg-white/90 p-7 shadow-[0_8px_30px_rgba(15,23,42,.04)] backdrop-blur transition duration-300 hover:-translate-y-1.5 hover:border-indigo-200 hover:shadow-[0_22px_50px_rgba(79,70,229,.12)] dark:border-slate-800 dark:bg-slate-900/90"><div className={`absolute -right-12 -top-12 h-32 w-32 rounded-full opacity-60 blur-2xl transition group-hover:scale-125 ${theme.glow}`} /><div className="relative flex items-start justify-between"><div className={`grid h-12 w-12 place-items-center rounded-2xl ${theme.icon}`}><Icon className="h-5 w-5" /></div><span className="rounded-full border border-slate-200 px-2.5 py-1 text-[9px] font-bold uppercase tracking-wider text-slate-400 dark:border-slate-700">{category.links.length} resources</span></div><h3 className="relative mt-6 text-lg font-extrabold tracking-tight">{category.name}</h3><p className="relative mt-2 min-h-12 text-sm leading-6 text-slate-500 dark:text-slate-400">{category.description}</p><a href={pathFor(category, category.links[0])} className="relative mt-6 flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-indigo-600 dark:text-indigo-400">Open collection <ArrowRight className="h-4 w-4 transition group-hover:translate-x-1" /></a></section>; })}</div>
  </div>;
}
